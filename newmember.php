<?php
    ...
    $memberdata = $memb->getUserData(Session::getSessionVar('userID'));
    if(Input::exists('post')) {    
        ...
        if($myVal->checkFormData($_POST, false)) {
            ...
            if(!$newID=$memb->addUser($userdetails)) {
                ...
                // Adding the user succeeded, set "haschildren" to 1 for the parent
                $memb->setFieldValue('haschildren',1,'membernum',$memberdata['membernum']);
            }
            ...
            $parentdetails = $memb->getUserData(Session::getSessionVar('userID'));
            $parentfullname = $parentdetails['firstname']." ".$parentdetails['middlename']." ".$parentdetails['lastname'];
            $parentfullname = str_replace("  "," ",$parentfullname);
            $parentfullname = $parentfullname." (".$parentdetails['gcnick'].")";
            ...
            $altbody=sprintf(_("A new submember has been registered on %s:"),Config::get('assocname'))."\r\n\r\n".
                                ...
                                sprintf(_("Created by:  %s"),$parentfullname)."\r\n\r\n".
                                sprintf(_("Please go to %s for more details."),Config::get('baseaddr'))."\r\n\r\n".
                                _("Best regards,")."\r\n".
                                Config::get('assocname');
            $body=sprintf(_("<p>A new submember has been registered on %s:"),Config::get('assocname'))."</p>".
                                ...
                                sprintf(_("<tr><th style=\"text-align:left\">Created by:</th><td>%s</td></tr>"),$parentfullname).
                                "</table>".
...
?>
                <div class="alert alert-info"> <?=_("You are a submember, so you cannot have your own submembers.")?></div>
<?php
        } else {
?>
                <div class="alert alert-info"> <?=_("This is a submember, which cannot have its own submembers.")?></div>
<?php
        }
    }
?>    
    ...
    <div class="col-md-10">
        <h3><?=_("Add new submember for")?> <?php echo $memberdata["username"] ?> (<?php echo $memberdata["firstname"]." ".$memberdata["middlename"]." ".$memberdata["lastname"] ?>)</h3>
        <p>
        <?=_("Submembers are just like any other member, and can log in to the member system and change their own details. The differences are:")?>
        <ul>
        <li><?=_("The \"main\" member's e-mail can be used for all submembers")?></li>
        <li><?=_("Only the \"main\" member receives fee notifications")?></li>
        <li><?=_("The \"main\" member is responsible for fee payment for all submembers")?></li>
        <li><?=_("The submember cannot remove the association to the main member")?></li>
        </ul>  
      <!-- column 2 -->	
        <hr/>
		<div class="row">
                ...
                <form class="form-horizontal" role="form" action="newmember.php" method="post">
<?php
...
?>
                    <div class="form-group<?php echo $errorstyle ?>">
                        <label for="username" class="col-sm-3 control-label"><?=_("Password")?></label>
                        <div class="col-sm-9">
...
?>
                    <div class="form-group<?php echo $errorstyle ?>">
                        <label for="username" class="col-sm-3 control-label"><?=_("Repeat password")?></label>
                        <div class="col-sm-9">
...
<?php
    $helptext = _("<div class='popoverhead'>Your e-mail address.</div>This is our primary communication channel, and required. For any submember, the main account's e-mail address will be used.");
...
?>
                    <div class="form-group<?php echo $errorstyle ?>">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" id="email" name="email" placeholder="<?=_("E-mail address")?>" value="<?php echo $memberdata["email"] ?>" data-trigger="focus" data-toggle="popover" data-placement="top" data-html="true" data-content="<?php echo $helptext; ?>"/>
                            <?php echo $errormsg ?>
                        </div>
                    </div>
<?php
    $helptext = _("<div class='popoverhead'>Your geocaching name.</div>For submembers, this can be the same as the main member, but it doesn't need to.");
...
?>
                    <div class="form-group<?php echo $errorstyle ?>">
                        ...
                        <div class="col-sm-4">
                            <div class="input-group date" id="dp1" data-date="" data-date-format="<?= Config::get('altdateformat') ?>">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <input type="hidden" id="parent" name="parent" value="<?php echo $memberdata["membernum"] ?>"/>
                            <button type="submit" class="btn btn-primary form-control"><?=_("Add submember")?></button>
                        </div>
                    </div>
                </form>
